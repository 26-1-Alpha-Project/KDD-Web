...
사용자가 코드를 수정하거나 접근 방식을 교정하면, **프로젝트 룰(`.claude/rules/`)에 반영**하여 같은 실수를 반복하지 않는다.

- 피드백은 반드시 `.claude/rules/` 파일에 규칙으로 추가한다
- 개인 memory에만 저장하고 프로젝트 룰에 반영하지 않는 것은 금지 — 다른 개발자가 레포를 클론했을 때도 동일하게 적용되어야 한다
- memory는 프로젝트 룰로 표현할 수 없는 사용자 개인 정보(역할, 선호도 등)에만 사용한다

### 피드백 유형별 처리

- "이거 아니야", "그거 말고" 같은 교정 → `.claude/rules/` 해당 파일에 금지 규칙 추가
- 기존 유틸/컴포넌트를 무시하고 새로 만든 경우 → `.claude/rules/architecture.md`에 재사용 규칙 추가
- API 타입을 임의로 변경한 경우 → `.claude/rules/api.md`에 올바른 타입 구조 기록
- Git/PR 관련 교정 → `.claude/rules/git.md`에 규칙 추가
